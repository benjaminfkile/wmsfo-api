...
// A26 test host - the api-key endpoints wired next to a representative sample
// of the /admin/* endpoints (sponsors + events + cookie types + settings) so
// the capability matrix has both an Editor endpoint (sponsors) and an Admin
// endpoint (events, cookie-types, settings) to point a key at.
public sealed class A26Host : IAsyncDisposable
{
    public WebApplication App { get; }
    public HttpClient Client { get; }
    public WmsfoOptions Options { get; }

    private A26Host(WebApplication app, HttpClient client, WmsfoOptions options)
    {
        App = app;
        Client = client;
        Options = options;
    ...
    public static async Task<A26Host> StartAsync(string connectionString)
    {
        ...
        {
            ApplicationName = typeof(A26Host).Assembly.GetName().Name,
        });
        ...
        // The three API-key endpoints under test.
        AdminApiKeyEndpoints.MapAll(app);
        // Representative admin surfaces the capability matrix points at.
        AdminSponsorEndpoints.MapAll(app);         // sponsors (Editor)
        ...
        return new A26Host(app, client, options);
    }
    ...
    public HttpRequestMessage KeyRequest(HttpMethod method, string path, string apiKey)
    {
        var req = new HttpRequestMessage(method, path);
        req.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue(
            "Bearer", apiKey);
