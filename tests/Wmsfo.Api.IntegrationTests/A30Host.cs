...
// A30 test host - admin events (with notify + clone), admin sponsors (with
// copy-from + import), and /me family (with /me/alerts) all wired to the real
// handlers. Adds an OutboxPublisher + AlertSender + FakeSesSender so the
// alert pipeline can be driven end-to-end from a status change or notify call.
public sealed class A30Host : IAsyncDisposable
{
    ...
    private A30Host(WebApplication app, HttpClient client, WmsfoOptions options,
        RecordingObjectStore store, FakeGatewayClient gateway, FakeSesSender sender,
        OutboxPublisher outbox, AlertSender alerts, WmsfoConnectionStrings connections)
    {
        App = app;
        Client = client;
        Options = options;
    ...
    public static async Task<A30Host> StartAsync(string connectionString)
    {
        ...
        {
            ApplicationName = typeof(A30Host).Assembly.GetName().Name,
        });
        ...
        return new A30Host(app, client, options, store, gateway, sender, outbox, alerts, connections);
    }
