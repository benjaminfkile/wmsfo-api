...
// A25 test host: events + routes + sponsors + settings admin endpoints wired
// to the real handlers so the [snapshot] frames actually build+PUT the
// snapshot with the pinned order, override, flight history and route poster
// changes from contracts 1.3.
public sealed class A25Host : IAsyncDisposable
{
    ...
    private A25Host(WebApplication app, HttpClient client, WmsfoOptions options,
    ...
    public static async Task<A25Host> StartAsync(string connectionString)
    {
        ...
        {
            ApplicationName = typeof(A25Host).Assembly.GetName().Name,
        });
        ...
        return new A25Host(app, client, options, store, gateway);
    }
