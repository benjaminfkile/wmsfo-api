...
// A minimal end-to-end host that mirrors Program.cs but points at the
// Postgres connection Testcontainers provides and swaps the object store /
// gateway client for in-memory fakes. Rate limits are disabled so the
// concurrent-inserts test cannot rate-limit itself out.
public sealed class A8Host : IAsyncDisposable
{
    ...
    private A8Host(WebApplication app, HttpClient client, WmsfoOptions options, RecordingObjectStore store, FakeGatewayClient gateway)
    {
        App = app;
        Client = client;
        Options = options;
    ...
    public static async Task<A8Host> StartAsync(string connectionString)
    {
        ...
        {
            ApplicationName = typeof(A8Host).Assembly.GetName().Name,
        });
        ...
        // We provide iconLibrary but disable path lookup: use the test paths dir.
        ...
        // A minimal rate limiter that adds every policy but makes them all unlimited
        // so the concurrent-inserts test isn't self-throttled.
        ...
        return new A8Host(app, client, options, store, gateway);
    }
