...
create table beacon (
  ...
  hub_allowed        boolean not null default true,   -- false: the authorize callback denies the join (contracts 2.4)
...
comment on column beacon.last_location_at is 'Last location the beacon delivered, published or not, stored as a row or carried (contracts 7.2). Health reads this: a beacon whose every fix repeats a stored position is still alive.';
